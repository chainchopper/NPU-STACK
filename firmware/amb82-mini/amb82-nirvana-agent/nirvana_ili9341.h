// Nirvana ILI9341 Driver — 4-wire SPI for Waveshare 2.4" LCD
// Controller: ILI9341, 240x320, RGB565. Target: AMB82-Mini (RTL8735B)
...
bool nirvana_display_init() {
    ...
    digitalWrite(TFT_RST,1);delay(10);digitalWrite(TFT_RST,0);delay(10);digitalWrite(TFT_RST,1);delay(150);
    _cmd(0x01);delay(130);_cmd(0x11);delay(130);
    _cmd(0xCF);_dat(0x00);_dat(0xC1);_dat(0x30);
    _cmd(0xED);_dat(0x64);_dat(0x03);_dat(0x12);_dat(0x81);
    _cmd(0xE8);_dat(0x85);_dat(0x00);_dat(0x78);
    _cmd(0xCB);_dat(0x39);_dat(0x2C);_dat(0x00);_dat(0x34);_dat(0x02);
    _cmd(0xF7);_dat(0x20);_cmd(0xEA);_dat(0x00);_dat(0x00);
    _cmd(0xC0);_dat(0x23);_cmd(0xC1);_dat(0x10);
    _cmd(0xC5);_dat(0x3E);_dat(0x28);_cmd(0xC7);_dat(0x86);
    _cmd(0x36);_dat(0x48);_cmd(0x3A);_dat(0x55);
    _cmd(0xB1);_dat(0x00);_dat(0x18);
    _cmd(0xB6);_dat(0x08);_dat(0x82);_dat(0x27);
...
void nirvana_display_fill(uint16_t c) {
    _win(0,0,TFT_WIDTH-1,TFT_HEIGHT-1);
    digitalWrite(TFT_CS,0);digitalWrite(TFT_DC,1);
    for(uint32_t i=0;i<(uint32_t)TFT_WIDTH*TFT_HEIGHT;i++)SPI.transfer16(c);
...
void nirvana_display_fill_rect(int16_t x,int16_t y,uint16_t w,uint16_t h,uint16_t c) {
    ...
    digitalWrite(TFT_CS,0);digitalWrite(TFT_DC,1);
    for(uint32_t i=0;i<(uint32_t)w*h;i++)SPI.transfer16(c);
    digitalWrite(TFT_CS,1);
}
void nirvana_text(int16_t x,int16_t y,const char* t,uint16_t fg,uint8_t sz) {
    while(*t){char c=*t++;if(c<' '||c>'~')c=' ';
        const uint8_t*g=F5x7[c-' '];
        for(int8_t co=0;co<5;co++){uint8_t ln=g[co];
            for(int8_t ro=0;ro<7;ro++)if(ln&(1<<ro)){
                if(sz==1)nirvana_display_fill_rect(x+co,y+ro,1,1,fg);
                else nirvana_display_fill_rect(x+co*sz,y+ro*sz,sz,sz,fg);
            }
        }x+=6*sz;
    }
}
...
void nirvana_page_status(const char* ip,int rssi,bool mq) {
    nirvana_display_fill(NIRVANA_BLACK);nirvana_header("NIRVANA FLEET");
    char b[32];nirvana_text(4,24,"Device:",0x8410,1);nirvana_text(58,24,NIRVANA_DEVICE_ID,0x07E0,1);
    nirvana_text(4,38,"Ver:",0x8410,1);nirvana_text(58,38,NIRVANA_VERSION,0x07FF,1);
    nirvana_text(4,56,"IP:",0x8410,1);nirvana_text(58,56,ip,0xFFFF,1);
    snprintf(b,sizeof(b),"%d dBm",rssi);nirvana_text(4,74,"WiFi:",0x8410,1);nirvana_text(58,74,b,rssi>-60?0x07E0:0xF800,1);
    nirvana_text(4,92,"MQTT:",0x8410,1);nirvana_text(58,92,mq?"ONLINE":"OFFLINE",mq?0x07E0:0xF800,1);
    nirvana_text(4,120,"Waveshare 2.4 ILI9341",0x8410,1);nirvana_text(4,134,"AMB82 RTL8735B + NN",0x8410,1);
    nirvana_center("NIRVANA FLEET",TFT_HEIGHT-40,0x8010,2);
}
void nirvana_page_network(const char* ip,int rssi) {
    nirvana_display_fill(NIRVANA_BLACK);nirvana_header("NETWORK");
    char b[32];nirvana_text(4,24,"SSID:",0x8410,1);nirvana_text(56,24,WiFi.SSID(),0x07FF,1);
    nirvana_text(4,42,"IP:",0x8410,1);nirvana_text(56,42,ip,0xFFFF,1);
    snprintf(b,sizeof(b),"%d dBm",rssi);nirvana_text(4,60,"RSSI:",0x8410,1);nirvana_text(56,60,b,0x07E0,1);
    nirvana_text(4,90,"5GHz WiFi + BLE 5.1",0x8410,1);
}
void nirvana_page_fleet(bool mq,unsigned long up) {
    nirvana_display_fill(NIRVANA_BLACK);nirvana_header("FLEET COMMS");
    char b[32];nirvana_text(4,24,"Protocol: MQTT+UDP",0x07E0,1);nirvana_text(4,42,"Topic: npu-fleet/amb82",0x8410,1);
    nirvana_text(4,60,"Broker:",0x8410,1);nirvana_text(60,60,MQTT_HOST,0x07FF,1);
    nirvana_text(4,82,"Status:",0x8410,1);nirvana_text(60,82,mq?"ONLINE":"OFFLINE",mq?0x07E0:0xF800,1);
    snprintf(b,sizeof(b),"%lu sec",up);nirvana_text(4,104,"Uptime:",0x8410,1);nirvana_text(60,104,b,0xFFFF,1);
}
#endif

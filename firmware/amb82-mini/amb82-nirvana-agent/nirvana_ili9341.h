// Nirvana ILI9341 Driver — 4-wire SPI, Waveshare 2.4" LCD
// Controller: ILI9341, 240x320 RGB565. Ameba SDK compatible.
...
bool nirvana_display_init(){
    ...
    // SPI: use transaction API (standard across all Arduino cores)
...
// ── Drawing (defined before page functions) ──
void nirvana_display_fill(uint16_t c){
    SPI.beginTransaction(SPISettings(40000000,MSBFIRST,SPI_MODE0));
...
// ── Page functions (no WiFi dependency — data passed as params) ──
void nirvana_page_status(const char* ip,const char* ssid,int rssi,bool mq){
    nirvana_display_fill(NIRVANA_BLACK);nirvana_header("NIRVANA FLEET");
    char b[32];
    nirvana_text(4,24,"Device:",0x8410,1);nirvana_text(58,24,NIRVANA_DEVICE_ID,0x07E0,1);
    ...
    nirvana_text(4,110,"Display: Waveshare 2.4\"",0x8410,1);
    nirvana_text(4,124,"IL9341 240x320 AMB82",0x8410,1);
...
void nirvana_page_network(const char* ip,const char* ssid,int rssi){
    ...
    nirvana_text(4,104,"RTL8735B 500MHz M33+NN",0x8410,1);
}
void nirvana_page_fleet(bool mq,unsigned long up){
    nirvana_display_fill(NIRVANA_BLACK);nirvana_header("FLEET COMMS");
    char b[32];
    nirvana_text(4,24,"Protocol: MQTT+UDP",0x07E0,1);
    nirvana_text(4,42,"Topic: npu-fleet/amb82",0x8410,1);

...
// Ameba SDK compatible — no beginTransaction needed, SPI.begin() in setup
...
{0x38,0x44,0x44,0x48,0x7F},{0x38,0x54,0x54,0x54,0x18},{0x08,0x7E,0x09,0x01,0x02},{0x08,0x54,0x54,0x54,0x3C},
...
// ── Init (SPI.begin() must be called BEFORE this in setup!) ──
bool nirvana_display_init(){
    pinMode(TFT_DC,OUTPUT);pinMode(TFT_RST,OUTPUT);pinMode(TFT_BL,OUTPUT);
    pinMode(TFT_CS,OUTPUT);  // CS after SPI.begin() — safe
    digitalWrite(TFT_CS,1);digitalWrite(TFT_BL,1);
    // Reset: LOW first (matches AmebaILI9341.cpp example)
...
void nirvana_splash(const char* ip,const char* ver){
    nirvana_display_fill(NIRVANA_BLACK);
    nirvana_center("NIRVANA FLEET",50,0x8010,2);
    nirvana_center("AMB82-Mini + ILI9341",80,0x07E0,1);
    nirvana_center(ver,105,0x07FF,1);
    if(ip[0]){nirvana_center(ip,140,0xFFFF,1);}
    nirvana_center("Nirvana True Intelligence",200,0x8410,1);
    nirvana_center("NPU-STACK Fleet",220,0x8410,1);
}

// ── Pages (params passed from sketch — no WiFi dependency) ──
void nirvana_page_status(const char* ip,const char* ssid,int rssi,bool mq,unsigned long up){
    ...
    nirvana_text(4,118,"Waveshare 2.4 ILI9341",0x8410,1);
    nirvana_text(4,132,"RTL8735B M33+NN 500MHz",0x8410,1);
    nirvana_text(4,150,"16MB Flash / 4MB PSRAM",0x8410,1);
...
void nirvana_page_network(const char* ip,const char* ssid,int rssi){
    ...
    nirvana_text(4,80,"5GHz WiFi + BLE 5.1",0x8410,1);
    nirvana_text(4,100,"OV5647 Camera CSI",0x8410,1);
    nirvana_text(4,114,"I2S Audio (Mic+Spkr)",0x8410,1);
    nirvana_text(4,128,"NN Engine (RTL8735B)",0x8410,1);
}
void nirvana_page_fleet(bool mq,unsigned long up){
    ...
    nirvana_text(4,130,"Nirvana Voice Ready",0x07E0,1);
    nirvana_text(4,144,"xiao zhi MQTT bridge",0x8410,1);
}
#endif
